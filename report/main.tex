...
\section{Player Hand/Move Detection}

\subsection{Objective}

The objective of this part is to identify precisely \textit{when} a player's hand enters and leaves the chessboard. This helps us know when a move is made.

\subsection{Methodology}

For this, we chose the \textbf{Background Subtraction} method using OpenCV's \texttt{MOG2} algorithm. This approach is more robust than simple frame differencing because it builds a statistical model of the static background (the empty board). A player hand that deviates significantly from this model is identified as ``foreground.''

Here's how it works:
\begin{enumerate}
    \item \textbf{Background Learning:} The system constantly observes the rectified chessboard (from Part 1) and learns what the ``normal'', static board looks like.
    \item \textbf{Motion Mask Generation:} When a hand enters the frame, the background subtractor compares the current image to its learned background model. It generates a ``motion mask'' -- a grayscale image where white pixels indicate areas of movement (the hand), and black pixels represent the static background. 
    (To ensure a clean mask, we configured the subtractor to \texttt{detectShadows=False}, which prevents shadows from being mistaken as motion.)
    \item \textbf{Contour Detection:} We then look for large contours in this motion mask. Small contours are ignored (\texttt{min\_area = 1000}) to filter out minor noise.
    \item \textbf{State Machine:} A simple state machine is used to track the interaction:
    \begin{itemize}
        \item \textbf{\texttt{Board is Stable}}: No hands are detected.
        \item \textbf{\texttt{Hand Detected}}: A hand (large contour) has entered the frame.
        \item \textbf{\texttt{Analyzing the Move}}: The hand has left the frame, returning the board to a stable state.
    \end{itemize}
\end{enumerate}


It's important to note that this part focuses solely on detecting the \textit{presence} and \textit{absence} of a hand. Determining \textit{which player} made the move will be handled in Part 3, by comparing the board state before and after the hand intervention.

\subsection{Results and Discussion}

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../figures/part2_summary.png}
    \caption{Hand Movement Detection Pipeline Summary}
\end{figure}

\begin{itemize}
    \item \textbf{Last Stable Board:} The rectified board just before the hand entered.
    \item \textbf{Motion Mask:} The binary mask generated by the background subtractor, clearly showing the detected hand in white.
    \item \textbf{Hand Contour Detected:} The final processed image, showing the original board with the detected hand outlined in green.
\end{itemize}

We have successfully implemented the detection of player hand movements via background subtraction. This allows us to know exactly when a move is being made, which is crucial for the next step of identifying the move itself.

\section{Pieces Detection and Identification}

\subsection{Objective}

The objective of this part is to accurately detect and identify the chess pieces on the board. This is essential for understanding the game state and the specific move being made.
